...
class ImageManager:
            ...
            if os.path.isdir(full_path):
                ...
                if item.endswith(".png"):
                    try:
                        ...
                        logging("ERROR", f"Failed to load image '{full_path}': {e}", "ImageManager._open_dir")
    ...
    def get_image(self, image_path: str, none_path: str="data/icons/none.png") -> py.Surface:
        ...
        if image is None:
            logging("ERROR", f"Image '{image_path}' not found in ImageManager", "ImageManager.get_image")
            image = self.images.get(none_path, None)
            if image is None:
                logging("ERROR", f"None image '{none_path}' not found in ImageManager", "ImageManager.get_image")
                try:
                    return py.image.load(image_path)
                except Exception as e:
                    logging("ERROR", f"Failed to load image '{image_path}': {e}", "ImageManager.get_image")
                    try:
                        return py.image.load(none_path)
                    except Exception as e:
                        logging("ERROR", f"Failed to load none image '{none_path}': {e}", "ImageManager.get_image")
    ...
    def get_worldcell_image(self, image_name: str, none_cat: str) -> py.Surface:
        ...
        if image is None:
            logging("ERROR", f"Worldcell image '{key}' not found in ImageManager", "ImageManager.get_worldcell_image")
