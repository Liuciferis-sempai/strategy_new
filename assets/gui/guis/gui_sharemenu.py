...
class GUIShareMenu:
        ...
        if self.share_target and self.share_starter:
            ...
            for i in range(self.share_starter.data.get("inventory_size", 1)):
                if i < len(self.share_starter.inventory):
                    self.share_starter_inventory.append([Icon(self.cell_size, self.cell_size, img=self.share_starter.inventory[i].name), TextField(text=str(self.share_starter.inventory[i].amount), font_size=50, width=0, height=0)]) #type: ignore
                else:
            ...
            if isinstance(target_inventory, list):
                ...
                for i in range(max_target_inventory): #type: ignore
                    if i < len(target_inventory):
                        self.share_target_inventory.append([Icon(self.cell_size, self.cell_size, img=target_inventory[i].name), TextField(text=str(target_inventory[i].amount), font_size=50, width=0, height=0)]) #type: ignore
                    else:
                ...
                for inventory_type in target_inventory.keys(): #type: ignore
                    ...
                    for i in range(max_target_inventory[inventory_type]): #type: ignore
                        if i < len(target_inventory[inventory_type]):
                            self.share_target_inventory[inventory_type].append([Icon(self.cell_size, self.cell_size, img=target_inventory[inventory_type][i].name), TextField(text=str(target_inventory[inventory_type][i].amount), font_size=50, width=0, height=0)]) #type: ignore
                        else:
    ...
    def click(self, cell: Icon, inventory_type: str, button: int):
        if inventory_type == "starter":
            for inv_cell, amount in self.share_starter_inventory:
                if inv_cell == cell:
                    if cell.img != "empty_inventory_cell.png": #type: ignore
                        if self.share_target.has_free_space(): #type: ignore
                            self.share_target.add_resource(cell.img, int(int(amount.text)//2) if button == 3 else int(amount.text)) #type: ignore
                            self.share_starter.remove_resource(cell.img, int(int(amount.text)//2) if button == 3 else int(amount.text)) #type: ignore
                            self.set_inventories()
                            update_gui()
                            return
        elif inventory_type == "target":
            if isinstance(self.share_target_inventory, list):
                for inv_cell, amount in self.share_target_inventory:
                    if inv_cell == cell:
                        if cell.img != "empty_inventory_cell.png": #type: ignore
                            if self.share_starter.has_free_space(): #type: ignore
                                self.share_starter.add_resource(cell.img, int(int(amount.text)//2) if button == 3 else int(amount.text)) #type: ignore
                                self.share_target.remove_resource(cell.img, int(int(amount.text)//2) if button == 3 else int(amount.text)) #type: ignore
                                self.set_inventories()
                                update_gui()
                                return
            elif isinstance(self.share_target_inventory, dict):
                for inventory_type in self.share_target.inventory.keys(): #type: ignore
                    for inv_cell, amount in self.share_target_inventory[inventory_type]: #type: ignore
                        if inv_cell == cell:
                            if cell.img != "empty_inventory_cell.png": #type: ignore
                                if self.share_starter.has_free_space(): #type: ignore
                                    self.share_starter.add_resource(cell.img, int(int(amount.text)//2) if button == 3 else int(amount.text)) #type: ignore
                                    self.share_target.remove_resource(cell.img, int(int(amount.text)//2) if button == 3 else int(amount.text), inventory_type) #type: ignore
                                    self.set_inventories()
                                    update_gui()
                                    return
        else:
    ...
    def draw(self):
        ...
        y_offset = 0
        x_offset = 0
        for cell, amount in self.share_starter_inventory:
            ...
            amount.change_position((root.cell_sizes[root.cell_size_scale][0]//2+self.share_starter_ico.width + (cell.width+10)*x_offset + (cell.width - amount.text_rect.width - 5), root.window_size[1]//2 - self.share_starter_ico.height//2 + (cell.height+10)*y_offset + amount.text_rect.height//2)) #type: ignore
            x_offset += 1

        y_offset = 0
        x_offset = 0
        if isinstance(self.share_target_inventory, list):
            for cell, amount in self.share_target_inventory:
                ...
                amount.change_position((root.window_size[0] - root.cell_sizes[root.cell_size_scale][0]//2 - self.share_target_ico.width - cell.width - (cell.width+10)*x_offset + (cell.width - amount.text_rect.width - 5), root.window_size[1]//2 - self.share_target_ico.height//2 + (cell.height+10)*y_offset + amount.text_rect.height//2)) #type: ignore
                x_offset += 1
        elif isinstance(self.share_target_inventory, dict):
            for i, inventory_type in enumerate(self.share_starter_inventory_org.keys()): #type: ignore
                ...
                y_offset += 1
                for cell, amount in self.share_target_inventory[inventory_type]:
                    if root.window_size[0] - root.cell_sizes[root.cell_size_scale][0]//2 - self.share_target_ico.width - cell.width - (cell.width+10)*x_offset < root.window_size[0]//2-root.cell_sizes[root.cell_size_scale][0]//4: #type:ignore
                        y_offset += 1
                        x_offset = 0
