...
def change_window_state(new_state:str):
    ...
    match root.window_state:
        ...
        case "fraction":
            ...
            root.handler.input_processor.process_mousemotion = root.handler.input_processor.process_mousemotion #not yet
        case "writing":
        ...
        case "inventory":
            ...
            root.handler.input_processor.process_mousemotion = root.handler.input_processor.process_mousemotion #not yet
        case _:
            logging("ERROR", f"Unknown window state: {root.window_state}", "change_window_state")
...
def start_the_game(game_name: str="New Game", game_seed: int=9999):
    ...
    root.handler.pawns_manager.add_resource(1, "resource_0", 2)

def logging(log_type: str, message: str, func_name: str="", extra_info: str=""):
    if func_name != "":
        message = f"{datetime.datetime.now()}: {log_type} [{func_name}] -> {message}; {extra_info}"
    else:
        message = f"{datetime.datetime.now()}: {log_type} {message}; {extra_info}"
    try:
