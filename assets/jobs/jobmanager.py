...
class JobManager:
    ...
    def _replace_target_in_args(self, job: dict, job_id: str, i: int = 0) -> dict:
        ...
        '''
        chosen_cell_coord = root.handler.get_chosen_cell_coord()
        target_coord = root.handler.gui.game.get_target_coord()
        ...
        if job["result"]["args"].get("target_cell", None) != None:
            job["result"]["args"]["target_cell"] = root.handler.world_map.get_cell_by_coord(target_coord)

        if job["result"]["args"].get("target_building_str", None) != None:
            building = root.handler.world_map.get_cell_by_coord(chosen_cell_coord).buildings
            job["result"]["args"]["target_building_str"] = building["name"].replace("scheme:of_", "")
            job["result"]["args"]["building_coord"] = chosen_cell_coord
            job["result"]["args"]["building_fraction"] = building["fraction_id"]
    ...
    def _is_job_available(self, job: dict, job_id: str, pawn: Pawn) -> bool:
        if hasattr(root.handler.trigger_manager, job["trigger"]["type"]):
            job = self._replace_target_in_args(job, job_id)
            trigger_func = getattr(root.handler.trigger_manager, job["trigger"]["type"])
